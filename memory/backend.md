...
5. **Боевые модульные правила и модульный промпт («Чистое LEGO»):**
   ...
   - **Архитектура модульного промпта (`src/ai/prompt_renderer.js` & `src/prompts.js`):**
     ...
     * Динамический контекст Radiant (время, погода, локация, статус, одежда, потребности) вынесен в модульный шаблон `prompt_radiant` с переменными `{{time}}`, `{{weather}}`, `{{location}}`, `{{needs}}`, `{{outfit}}`, `{{status}}`.
