...
1. **Native Tool Calling (`src/radiant/actions/` & `src/ai.js`):**
   ...
   - **Отложенные обещания и действия (`schedule_followup.js`):**
     ...
     - **Умная валидация свежести (Freshness Guard):** входящее сообщение юзера больше не отменяет обещание вслепую (чтобы не сбрасывать таймер на обычное «давай/ок»). При срабатывании таймера в `queue.js` воркер проверяет свежесть диалога: если диалог активен (< 3 мин) и обещание было статусным («душ/сон»), возврат тихо пропускается; для контентных обещаний («скину кофе/фотку») или при паузе в чате возврат срабатывает в штатном режиме.
     - **Досрочное погашение контентных обещаний:** если в ходе диалога Лера уже отправила обещанное фото или войс, отложенная джоба автоматически гасится из `ai.js`, исключая дублирование.
...
5. **Боевые модульные правила и модульный промпт («Чистое LEGO»):**
   - Правила профиля (`Личка / Casual`, `Личка / Erotic 18+`, `Инициатива`, `Тг-канал`) напрямую управляют генерацией ответа.
   - **Архитектура модульного промпта (`src/ai/prompt_renderer.js` & `src/prompts.js`):**
     * Системный промпт собирается СТРОГО и ИСКЛЮЧИТЕЛЬНО из модулей, прикрепленных к активному правилу (`rule.attachedPromptIds`).
     * Никаких скрытых нередактируемых инъекций: если у правила откреплены все модули (`attachedPromptIds: []`), отправляемый системный промпт строго пустой (`""`, 0 символов).
     * Динамический контекст Radiant и собеседника вынесен в модульный шаблон `prompt_radiant` с переменными `{{time}}`, `{{weather}}`, `{{location}}`, `{{needs}}` (`{{wellbeing}}`), `{{outfit}}`, `{{status}}`, `{{channel_stats}}`, `{{day_events}}`, `{{relationship}}`, `{{situation}}`, `{{user_name}}`, `{{radiant_context}}`. Все данные вычисляются через `ContextBuilder.buildTemplateVariables` и отдаются в модель без потери модульности — любой блок можно отредактировать или удалить в админке.
     * Долгосрочная память вынесена в модульный шаблон `prompt_memory` с переменной `{{memory_facts}}`.
     * Антиповторы и ограничения вынесены в модуль `prompt_antirep`.
     * **Шлюз нативных инструментов (Tool Calling):** схемы `tools` передаются в нейросеть ТОЛЬКО если к правилу прикреплен модуль инструментов (`prompt_tools`, `routing_common` или с флагом `is_tool_module`). Если модуль откреплен — `generationParams.tools` не передается (`tools: null`), отключая вызов функций для этого правила.
...
- **Инициативы и контент (Initiatives & Content Delivery):**
  - Защита от спам-лупов: `consecutive_initiatives >= 1` блокирует утренний `new_day` до ответа пользователя.
