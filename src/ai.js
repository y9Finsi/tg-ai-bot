    ...
    if ((isInitiative && judgeNeedsRetry) || (!isInitiative && userText && (
        ...
        const retryInstruction = qualityIssues.includes('format')
            ...
            ? judgeResult.code === 'INVENTED_FACT'
                ? 'СТОП: проверка качества отклонила предыдущий ответ (INVENTED_FACT). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО выдумывать прошлые разговоры собеседника, которых нет в памяти и истории! Перепиши ответ, говори только о себе и своих реальных делах или отреагируй на последнюю фразу.'
