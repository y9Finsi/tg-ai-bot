...
- Полная вовлечённость в процесс, живость ощущений, отклик на действия партнёра без нытья и прерываний.`;
    ...
    if (baseSystemPromptText?.isModular) {
        ...
        if (routingMode === 'EROTIC' && systemPrompt) {
            const climaxPrompt = getClimaxPromptInstruction(climaxState);
            if (climaxPrompt && !systemPrompt.includes('ЭТАП БЛИЗОСТИ')) {
                systemPrompt = `${systemPrompt}\n\n[РЕЖИМ БЛИЗОСТИ И ВИРТА]:${climaxPrompt}`.trim();
            }
        }
    } else {
...
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: писать шаблонные фразы вроде «я вернулась как и обещала». Пиши естественно, по-питерски, как живая 19-летняя девушка.`;
        ...
        const triggerText = initiativeDirective 
            ? `[СИСТЕМНОЕ СОБЫТИЕ: ${initiativeReason || 'Инициативное сообщение'}]\n${initiativeDirective}`
            : `[СИСТЕМНОЕ СОБЫТИЕ: ${initiativeReason || 'Напиши сообщение первой'}]`;
            ...
            if (settled.status === 'fulfilled') {
                ...
                if (name === 'send_photo') {
                    ...
                    if (execRes?.status === 'success') {
                        ...
                        messages.push({
                            role: 'system',
                            content: '[МАТЕРИАЛ НЕДОСТУПЕН]: Материал недоступен. Ответь собеседнику своими словами от лица Леры.'
                        });
                    }
                } else {
                    messages.push({
