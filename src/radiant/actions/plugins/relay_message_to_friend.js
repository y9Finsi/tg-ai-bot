...
export const relayMessageToFriendAction = {
    ...
    description: 'Передает сообщение, просьбу, привет или реплику общему знакомому или другу («передай Богдану, что...», «черкани Юте...», «напиши юте что...», «скажи @username...», «передай ему/ей в ответ...»). Вызывай ОБЯЗАТЕЛЬНО, когда пользователь просит передать или написать что-то другому человеку. Не путай собеседника с получателем! Поддерживает задержку, чтобы выглядеть живо.',
    inputSchema: {
        type: 'object',
        properties: {
            target: {
                type: 'string',
                description: 'Имя человека или @username (например: "Богдан", "Ютя", "@bogdan"). Если пользователь отвечает на полученное от кого-то сообщение («передай ему...», «ответь ей...»), укажи "ему" или имя того человека.'
            },
            message: {
                type: 'string',
