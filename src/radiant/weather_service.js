...
export class WeatherService {
    ...
    static async getSnapshot({ fetcher = globalThis.fetch, now = Date.now() } = {}) {
        ...
        try {
            ...
            cached = { is_raining: Number(current.rain || current.precipitation || 0) > 0 || [51, 53, 55, 61, 63, 65, 80, 81, 82].includes(Number(current.weather_code)), temperature_c: current.temperature_2m ?? null, weather_code: current.weather_code ?? null, status: 'fresh', fetched_at: new Date(now).toISOString(), coordinates: SPB };
            return cached;
        } catch (error) {
            if (cached && now - new Date(cached.fetched_at).getTime() <= TTL_MS) return { ...cached, status: 'stale', error: error.message };
