...
export function buildImagePrompt({ prompt, baseStyle, negativePrompt, hasReference = false, isChatModel = true }) {
    const defaultBaseStyle = `**REFERENCE & IDENTITY:**
...
NEGATIVE: Professional photography, studio lighting, DSLR, 8k, ultra sharp, cinematic lighting, beauty retouch, airbrushed skin, oversharpened details, high micro-contrast, professional color grading, artificial skin, plastic texture, perfect symmetry, glamour portrait, fashion editorial, unrealistic anatomy, distorted hands, excessive bokeh, dramatic lighting, overly polished image.`;
    ...
    if (hasReference) {
        return [
            ...
            style,
            cleanPrompt ? `[CURRENT SCENE & OUTFIT / DYNAMIC OVERRIDE]:\n${cleanPrompt}\n(Render character wearing exactly this specified outfit and in this environment while strictly preserving character identity, facial bone structure, freckles, eyeliner, and authentic mobile photo aesthetic).` : '',
