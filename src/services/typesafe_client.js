...
export function buildTypeSafeClassifierQuestions({ allowReaction = true, includeToolPlan = true } = {}) {
    ...
    if (includeToolPlan) {
        ...
        questions.primary_tool = {
            type: 'choice',
            instructions: 'Определи главный инструмент или отсутствие инструмента для новой реплики. Проверь предыдущие реплики на тулы и текущую на тул: 1) Если на предыдущих репликах Лера пообещала что-то прислать/показать/записать («скину», «пришлю», «послушай», «держи», трек, фото, ссылку, войс), но инструмент в истории НЕ вызывался — ОБЯЗАТЕЛЬНО выбери этот инструмент (send_content, send_photo, send_voice) прямо сейчас для выполнения обещания со source PREVIOUS_PROMISE. 2) Если на текущей реплике пользователь просит или разговор требует контента/фото/действия — выбери инструмент из toolCatalog со source CURRENT_MESSAGE. 3) Иначе выбери NONE. Не выбирай tool для уже выполненного обещания.',
        ...
        questions.promise_recovery = {
            type: 'noul',
            instructions: 'Нужно ли сейчас выполнить невыполненное обещание Леры из предыдущих реплик или pendingPromises (прислать трек, ссылку, фото, войс)?',
            criteria: { true: 'Есть невыполненное обещание Леры без подтверждённого успешного tool event.', false: 'Невыполненного обещания нет или оно уже выполнено.' }
        };
...
export function buildTypeSafeJudgeQuestions() {
    return {
        ...
        unfulfilled_tool: {
            type: 'choice',
            instructions: 'Проанализируй текст кандидата-ответа Леры (candidateReply) и список реально вызванных инструментов (toolTrace в state). Содержит ли реплика Леры слова или обещание прислать, скинуть, показать или послушать трек, песню, фото, ссылку, видео или войс (например: «скину», «пришлю», «держи», «лови», «вот ссылка», «послушай трек»), при том что соответствующий инструмент (send_content, send_photo, send_voice) НЕ БЫЛ вызван в toolTrace и в тексте нет реальной ссылки? Выбери инструмент, который требуется вызвать прямо сейчас, либо NONE.',
            criteria: {
                NONE: 'В тексте нет невыполненных обещаний медиа/контента, либо инструмент уже успешно вызван, либо в тексте есть реальная ссылка.',
                send_content: 'В тексте есть обещание или слова «скину/пришлю/послушай трек/музыку/ссылку/статью/мем/видео», но send_content НЕ был вызван.',
                send_photo: 'В тексте есть обещание скинуть или показать фото/селфи/лук, но send_photo НЕ был вызван.',
                send_voice: 'В тексте есть обещание записать или скинуть голосовое/войс, но send_voice НЕ был вызван.'
            }
        },
        relationship_event: {
            type: 'choice',
