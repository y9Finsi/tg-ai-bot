...
- Отклоняй (REJECT:CHANNEL_CLICHE), только если пост прямо начинается с шаблонных зачинов позы («сижу на кухне/подоконнике», «еду в маршрутке», «валяюсь под пледом», «знаете что...», «а вы знали...»). Обычные бытовые глаголы внутри текста — это НЕ клише.
- Отклоняй (REJECT:CHANNEL_REPETITION), только если сюжет или шутка дословно повторяют недавние опубликованные посты из списка ниже.
- Отклоняй (REJECT:CHANNEL_INVENTED_FACT), если пост содержит конкретные вымышленные события/новости, противоречащие публичным фактам.
...
- Отклоняй (REJECT:CHANNEL_FORMAT), если пост содержит списки, дефисы в начале строк или эмодзи.
- Отклоняй (REJECT:CHANNEL_FORMAT_MISMATCH), если пост не соответствует формату ${contentFormat || 'обычного наблюдения'}.
- Отклоняй (REJECT:CHANNEL_TOO_LONG), если пост длиннее 800 символов или выглядит как заумная статья.
- Редакционный режим: ${editorialMode}. В режиме reference_short допустимы короткая мысль, фото-подпись и бытовое наблюдение.
...
Для отказа используй только channel-коды. Если пост живой, ироничный, написан от первого лица без канцелярита и без бреда — ОБЯЗАТЕЛЬНО верни PASS.`
        : '';
