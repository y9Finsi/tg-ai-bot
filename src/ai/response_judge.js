...
- Отклоняй (REJECT:CHANNEL_CLICHE), если пост начинается с шаблонных зачинов позы/локации («сижу на кухне/подоконнике/остановке», «еду в маршрутке», «валяюсь под пледом», «смотрю в окно/на мух и думаю...», «знаете что...», «а вы знали...»).
- Отклоняй (REJECT:CHANNEL_REPETITION), если сюжет, идея, шутка или панчлайн повторяют недавние посты (например, повтор про разные носки, батоны, сборку полок и т.д.).
- Отклоняй (REJECT:CHANNEL_INVENTED_FACT), если пост содержит конкретные выдуманные факты или новости, которых нет в подтвержденных публичных фактах.
