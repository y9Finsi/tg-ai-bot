...
async function main() {
    ...
    const tmpDir = '/tmp/chrome_sunset_test2_' + Date.now();
    ...
    // Poll for pattern to become 'spb-facade-sunset' over 6 seconds
    let finalPattern = null;
    let finalRoadUnder = false;
    for (let i = 0; i < 30; i++) {
        await new Promise(r => setTimeout(r, 200));
        ...
        finalPattern = audit.result.value.pattern;
        finalRoadUnder = audit.result.value.roadUnderBuildings;
        if (finalPattern === 'spb-facade-sunset' && finalRoadUnder) {
            console.log(`Matched after ${(i + 1) * 200}ms!`);
            break;
        }
    }

    console.log('Final Sunset Pattern:', finalPattern, 'RoadUnderBuildings:', finalRoadUnder);
