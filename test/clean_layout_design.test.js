...
test('admin v2 styles contain frameless container rules, 32px vertical section gaps, and horizontal bento layout', () => {
    ...
    assert.ok(css.includes('ui-card-frameless'), 'styles.css should define ui-card-frameless class');

    // Check increased vertical section spacing (32px bottom margins)
    assert.ok(css.includes('margin-bottom:32px'), 'styles.css should specify 32px bottom margins for main section cards');

    // Check horizontal Bento grid layout rules and responsive breakpoints
    assert.ok(css.includes('bento-needs-layout'), 'styles.css should contain bento-needs-layout');
    assert.ok(css.includes('@media(max-width:950px)'), 'styles.css should contain 950px responsive breakpoint');
